...
- Focus rings on the scanner controls are drawn inside the control, not around it. An outline sits outside the border box, which over a live feed means no colour is safe: a dark ring vanished against the idle lens, and a white one would vanish against a bright shot. Inside, each ring only ever meets its own button's fill.
