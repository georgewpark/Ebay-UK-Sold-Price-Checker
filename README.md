...
- Every colour pair in `src/index.css` is chosen against a measured contrast ratio, including the boundary colours that 1.4.11 covers, not just text. The scan button is the exception, because it sits on the camera feed and stays white in either scheme, so its colours are literals rather than tokens. That is exactly how they once slipped past this rule, so measure any hex outside `index.css` by hand.
- Focus rings on the scanner controls are drawn inside the control, not around it. An outline sits outside the border box, which over a live feed means no colour is safe: a dark ring vanished against the idle lens, and a white one would vanish against a bright shot. Inside, each ring only ever meets its own button's fill. The reticle carries a drop shadow for the same reason, since the accent alone falls to about 1.3:1 over a pale label.
- Scanner state is announced: routine updates politely, errors assertively.
- The flash that confirms a scan is rate limited, because in keep-scanning mode nothing else holds one back. Reads come in as fast as the camera delivers frames, and a flash per read is a strobe over most of the screen rather than a confirmation: the accent at 85% over a dark shot moves relative luminance by about 0.5, and the viewfinder is far larger than the small safe area WCAG 2.3.1 exempts. `FLASH_GAP_MS` in [src/components/Viewfinder.tsx](src/components/Viewfinder.tsx) caps it at two a second, against the three the criterion allows. Only the flash waits; the barcode, the lookup and the history row still land on every read. Note that `prefers-reduced-motion` collapsing the animation is not a fix here, since 2.3.1 has to hold by default.
- The duplicate guard in continuous mode keeps a gap per code rather than one slot for the code read last. A single slot only ever caught an immediate repeat, so two barcodes inside the reticle were read in turn, A, B, A, B, and neither ever looked like a repeat: the pair fired a detect on every frame for as long as both stayed in view. A multipack, a shelf edge, or one good read alternating with a misread of the same label is enough to do it. See [src/lib/duplicates.ts](src/lib/duplicates.ts).
...
- Nothing that disappears leaves focus stranded, whether the user asked for it to go or not. Clearing the history moves focus to its heading, and removing one row moves it to the row that took its place, along with a spoken confirmation naming what went and how many scans are left. That count is not decoration: two rows can carry the same label, and a message repeated word for word writes nothing to the DOM, so the second removal would be announced to nobody. A successful scan releases the camera, which takes the torch pill with it, so focus moves to the scan button rather than to `<body>`. Recording that scan also rebuilds the row of anything already in the list, so focus goes back to the button it was on. Note that a browser blurs an element when it is merely moved in the DOM, not only when it is removed, so keeping a stable key would not save any of these on its own.
...
- Anything the eye gets, the ear gets. No information is left in a `title` attribute, which never appears on touch and cannot be reached by keyboard. It runs the other way too: history rows wrap long product names rather than truncating them, because an ellipsis puts the rest of the name out of reach of the one reader a screen reader cannot help.
...
src/
  ...
  lib/
    ...
    duplicates.ts       Which reads in continuous mode are a new item
